...
class MyDB extends SQLite3
{
    ...
    function getIncomeForMinusDay($minusDay) {
        ...
        $incomeSql = $this->prepare('select (select acc_grid_output from readings where "timestamp" between :timestamp and :now order by "timestamp" desc limit 1) - (select acc_grid_output from readings where "timestamp" < :timestamp order by "timestamp" desc limit 1);');
        $expensesSql = $this->prepare('select (select acc_grid_input from readings where ("timestamp" between :timestamp and :now) order by "timestamp" desc limit 1) - (select acc_grid_input from readings where "timestamp" < :timestamp order by "timestamp" desc limit 1);');
